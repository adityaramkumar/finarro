...
import { accountsApi, transactionsApi, dashboardApi, sharesApi } from '../services/api';
...
const DashboardPage = () => {
  ...
  const handleShareChart = async () => {
    try {
      ...
      const displayName = user?.pseudonymous_username || user?.first_name || 'My';
    ...
    <div className="space-y-8 animate-fade-in">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-4xl font-bold text-white">
            Welcome back, <span className="text-transparent bg-gradient-to-r from-indigo-400 to-purple-400 bg-clip-text">{user?.first_name}</span>!
          </h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4 2xl:gap-6">
        <div className="bg-gray-950/30 backdrop-blur border border-gray-800/50 rounded-2xl p-4 hover:bg-gray-950/50 hover:border-gray-700/50 transition-all duration-300 group hover:shadow-lg hover:shadow-green-500/5">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <p className="text-xs text-gray-400 mb-1 group-hover:text-gray-300 transition-colors">Total Balance</p>
            ...
            <div className={`flex items-center space-x-1 text-xs font-medium ${(summary.balance_change || 0) >= 0 ? 'text-green-400' : 'text-red-400'}`}>
              {(summary.balance_change || 0) >= 0 ? <ArrowUpRight className="h-3 w-3" /> : <ArrowDownRight className="h-3 w-3" />}
              <span>
                {isLoading ? (
        ...
        <div className="bg-gray-950/30 backdrop-blur border border-gray-800/50 rounded-2xl p-4 hover:bg-gray-950/50 transition-all duration-300 group">
          <div className="flex items-center justify-between">
            ...
            <div className={`flex items-center space-x-1 text-xs font-medium ${(summary.monthly_income_change || 0) >= 0 ? 'text-green-400' : 'text-red-400'}`}>
              {(summary.monthly_income_change || 0) >= 0 ? <ArrowUpRight className="h-3 w-3" /> : <ArrowDownRight className="h-3 w-3" />}
              <span>
                {isLoading ? (
                  <div className="h-4 w-12 bg-gray-700/50 rounded animate-pulse"></div>
                ) : (
        ...
        <div className="bg-gray-950/30 backdrop-blur border border-gray-800/50 rounded-2xl p-4 hover:bg-gray-950/50 transition-all duration-300 group">
          <div className="flex items-center justify-between">
            ...
            <div className={`flex items-center space-x-1 text-xs font-medium ${(summary.monthly_expenses_change || 0) >= 0 ? 'text-red-400' : 'text-green-400'}`}>
              {(summary.monthly_expenses_change || 0) >= 0 ? <ArrowUpRight className="h-3 w-3" /> : <ArrowDownRight className="h-3 w-3" />}
              <span>
                {isLoading ? (
                  <div className="h-4 w-12 bg-gray-700/50 rounded animate-pulse"></div>
                ) : (
        ...
        <div className="bg-gray-950/30 backdrop-blur border border-gray-800/50 rounded-2xl p-4 hover:bg-gray-950/50 transition-all duration-300 group">
          <div className="flex items-center justify-between">
            ...
            <div className={`flex items-center space-x-1 text-xs font-medium ${(summary.investment_returns_change || 0) >= 0 ? 'text-green-400' : 'text-red-400'}`}>
              {(summary.investment_returns_change || 0) >= 0 ? <ArrowUpRight className="h-3 w-3" /> : <ArrowDownRight className="h-3 w-3" />}
              <span>
                {isLoading ? (
                  <div className="h-4 w-12 bg-gray-700/50 rounded animate-pulse"></div>
                ) : (
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="text-xl font-bold text-white">{formatCurrency(summary.net_growth || 0)}</p>
              </div>
            </div>
      ...
      <div className="bg-gray-950/30 backdrop-blur border border-gray-800/50 rounded-2xl p-8 hover:border-gray-700/50 transition-all duration-300 hover:shadow-lg hover:shadow-indigo-500/5">
        <div className="flex items-center justify-between mb-8">
          <div>
            <h2 className="text-2xl font-bold text-white">Net Worth Over Time</h2>
            <p className="text-gray-400 mt-2">Track your financial growth and overall wealth progression</p>
          </div>
          <div className="flex items-center space-x-8">
            ...
            <div className="text-right">
              <p className="text-sm text-gray-400">Current Net Worth</p>
                              <p className="text-xl font-bold text-white">{formatCurrency(netWorthData.length > 0 ? netWorthData[netWorthData.length - 1].netWorth : 0)}</p>
            </div>
          </div>
        </div>
        <div className="h-96">
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center h-full text-center">
              ...
              <h3 className="text-lg font-semibold text-white mb-2">No Financial Data Available</h3>
              <p className="text-gray-400 max-w-md">
                Connect your bank accounts and add transactions to see your net worth trends and financial growth over time.
              </p>
      ...
      <div className="bg-gray-950/30 backdrop-blur border border-gray-800/50 rounded-2xl p-6 hover:border-gray-700/50 transition-all duration-300">
        <div className="flex items-center justify-between">
          <div>
            <h3 className="text-lg font-semibold text-white mb-2">Share Your Progress</h3>
            <p className="text-gray-400 text-sm">
              Create a shareable link to show your net worth growth. Only high-level chart data is shared - no sensitive information.
            </p>
          </div>
          <div className="flex items-center space-x-4">
            <button
            ...
            >
              ...
              <span>{isSharing ? 'Creating Link...' : 'Generate Share Link'}</span>
            </button>
          </div>
        </div>
          ...
          <div className="mt-6 p-4 bg-gray-800/50 rounded-xl border border-green-500/20">
            <div className="flex items-center justify-between">
              <div className="flex-1">
                <p className="text-green-400 font-medium text-sm mb-2">✅ Share link ready!</p>
                <div className="flex items-center space-x-3">
                  <input
                    type="text"
                    value={shareUrl}
            ...
            <p className="text-gray-500 text-xs mt-3">
              Anyone with this link can view your net worth chart without logging in. No personal or account information is shared.
            </p>
          </div>
        )}
      </div>
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between p-4 bg-gray-800/20 rounded-xl border border-green-500/10">
              ...
              <span className="text-lg font-semibold text-white">
                {formatCurrency(netWorthData.length > 0 ? netWorthData[netWorthData.length - 1].assets : 0)}
              </span>
            </div>
            <div className="flex items-center justify-between p-4 bg-gray-800/20 rounded-xl border border-red-500/10">
              ...
              <span className="text-lg font-semibold text-white">
                {formatCurrency(netWorthData.length > 0 ? netWorthData[netWorthData.length - 1].liabilities : 0)}
              </span>
            </div>
            <div className="flex items-center justify-between p-4 bg-gradient-to-r from-green-500/10 to-emerald-500/10 rounded-xl border border-green-500/30">
              <div className="flex items-center">
                ...
                <span className="text-sm font-semibold text-green-400">Net Worth</span>
              </div>
              <span className="text-lg font-bold text-green-400">
                {formatCurrency(netWorthData.length > 0 ? netWorthData[netWorthData.length - 1].netWorth : 0)}
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-2 bg-gray-950/30 backdrop-blur border border-gray-800/50 rounded-2xl p-6">
          ...
          {spendingByCategory.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-80 text-center">
              ...
              <p className="text-gray-400 max-w-md">
                Start making transactions to see your spending patterns and category breakdowns.
              </p>
              <div className="mt-6">
                <button className="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg transition-colors">
                  Add Transaction
                ...
                <div className="grid grid-cols-2 gap-4">
                  {spendingByCategory.map((category, index) => (
                    <div key={index} className="flex items-center justify-between p-3 bg-gray-800/20 rounded-lg">
                      ...
                      <span className="text-sm font-semibold text-white">{formatCurrency(category.value)}</span>
                    </div>
                  ))}
                </div>
              </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-gray-950/30 backdrop-blur border border-gray-800/50 rounded-2xl p-6">
          <div className="flex items-center justify-between mb-6">
            ...
            <button className="text-indigo-400 hover:text-indigo-300 text-sm font-medium transition-colors hover:underline">View All</button>
          </div>
          {accounts.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-64 text-center">
              ...
              <p className="text-gray-400 max-w-md">
                Connect your bank accounts to track balances and see your complete financial picture.
              </p>
              <div className="mt-6">
                <button className="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg transition-colors">
                  Connect Account
            ...
            <div className="space-y-4">
              {accounts.map((account) => (
                <div key={account.id} className="group flex items-center justify-between p-5 bg-gray-800/20 rounded-xl hover:bg-gray-800/40 transition-all duration-300 border border-gray-700/20 hover:border-gray-600/40">
                  <div className="flex items-center space-x-4">
                    ...
                    <div className="min-w-0 flex-1">
                      <p className="font-semibold text-white truncate text-base">{account.name}</p>
                      <p className="text-sm text-gray-400 capitalize mt-1">{account.type} account</p>
                    </div>
                  </div>
                  <div className="text-right">
                    <p className="font-bold text-white text-lg">{formatCurrency(account.balance)}</p>
                    <p className={`text-sm font-medium mt-1 ${account.change >= 0 ? 'text-green-400' : 'text-red-400'}`}>
        ...
        <div className="bg-gray-950/30 backdrop-blur border border-gray-800/50 rounded-2xl p-6">
          ...
          {transactions.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-64 text-center">
              ...
              <p className="text-gray-400 max-w-md">
                Start tracking your financial activity by adding transactions or connecting accounts.
              </p>
              <div className="mt-6">
                <button className="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg transition-colors">
                  Add Transaction
            ...
            <div className="space-y-4">
              {transactions.map((transaction) => (
                <div key={transaction.id} className="group flex items-center justify-between p-5 bg-gray-800/20 rounded-xl hover:bg-gray-800/40 transition-all duration-300 border border-gray-700/20 hover:border-gray-600/40">
                  <div className="flex items-center space-x-4">
                    <div className={`h-14 w-14 rounded-xl flex items-center justify-center shadow-lg group-hover:shadow-xl transition-shadow ${
                    ...
                    <div className="min-w-0 flex-1">
                      <p className="font-semibold text-white truncate text-base">{transaction.description}</p>
                      <p className="text-sm text-gray-400 mt-1">{transaction.category} • {transaction.date}</p>
                    </div>
                  </div>
                  <div className="text-right">
